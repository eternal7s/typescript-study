...
- 🦄 모듈 이해하기 [:link:](#-모듈-이해하기)
  - 🐇 runsServer.ts 파일의 모듈화 [:link:](#-indexts-파일의-모듈화)
...
- `dev` 명령을 개발 중에 `src` 디렉터리에 있는 `runsServer.ts` 파일을 실행하는 용도로 사용하며, `build` 명령은 개발이 완료된 후 프로그램을 배포하기 위해 `dist` 디렉터리에 ES5 자바스크립트 파일을 만들 때 사용한다.
...
## 🦄 모듈 이해하기
- 타입스크립트에서는 `runsServer.ts`와 같은 소스 파일을 모듈(module)이라고 한다.
...
- 터미널에서 다음 명령으로 `runsServer.ts` 파일을 실행한다.
...
- `runsServer.ts` 파일의 내용을 분리해서 모듈화를 진행한다.

### 🐇 runsServer.ts 파일의 모듈화
- `runsServer.ts` 파일을 모듈화하기 위해 `src` 디렉터리 아래에 `person` 디렉터리를 생성 후 그 안에 `Person.ts`라는 이름의 파일을 만든다.
- 그리고 `runsServer.ts` 파일에서 다음과 같은 내용을 `Person.ts` 파일로 옮겨 적는다.
